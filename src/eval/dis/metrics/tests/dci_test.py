...
import src.eval.dis.metrics.tests.dummy_data as dummy_data
import src.eval.dis.metrics.dci as dci 
import numpy as np
...
class DCITest(absltest.TestCase):
  ...
  def test_metric(self):
    ...
    dci_metric = dci.DciMetric(dataset=ground_truth_data, 
                               random_state=random_state, n_points=1000)
    
    scores = dci_metric(FakeModel(representation_function))
    self.assertBetween(scores["dmetric/dci_disentanglement"], 0.9, 1.0)
    self.assertBetween(scores["dmetric/dci_completeness"], 0.9, 1.0)
  ...
  def test_bad_metric(self):
    ...
    dci_metric = dci.DciMetric(dataset=ground_truth_data, 
                               random_state=random_state, n_points=1000)
    scores = dci_metric(FakeModel(representation_function))
    self.assertBetween(scores["dmetric/dci_disentanglement"], 0.0, 0.2)
    self.assertBetween(scores["dmetric/dci_completeness"], 0.0, 0.2)
  ...
  def test_duplicated_latent_space(self):
    ...
    self.assertBetween(scores["dmetric/dci_completeness"], target-.1, target+.1)
