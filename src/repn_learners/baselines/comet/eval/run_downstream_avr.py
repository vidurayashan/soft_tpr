...
def main(args):
    ...
    param_count = 0 
    for ebm_model in ebm_models:
        param_count += sum(p.numel() for p in ebm_model.parameters() if p.requires_grad)
    ...
    for n_samples_of_interest in args.n_sample_list.split(','):
        if n_samples_of_interest == '100000': 
            ...
            models.append(WReN(embedding_fn=embedding_fn, embedding_dim=args.latent_dim, 
