...
The below code is adapted from https://github.com/ThomasMrY/VCT/blob/master/main_vct.py
...
import gin
import itertools
import src.repn_learners.baselines.vct.dataset as dataset
from torchvision import datasets, transforms
import src.repn_learners.baselines.vct.eval.eval_dis_metrics as evaluate

from imageio import imwrite
from torchvision.utils import make_grid

from src.repn_learners.baselines.vct.models.autoencoder import *
import math
import random
import src.repn_learners.baselines.vct.data.ground_truth.shapes3d as dshapes3d
import src.repn_learners.baselines.vct.data.ground_truth.mpi3d as dmpi3d
import src.repn_learners.baselines.vct.data.ground_truth.dsprites as ddsprit
import src.repn_learners.baselines.vct.data.ground_truth.cars3d as dcars3d
...
def train(args, train_loader, model, decoder_model, latent_encoder, optimizer, eval_dataset, test_loader):
    ...
    for epoch in range(train_flags.num_epochs):
        for batch_idx, (data, _) in enumerate(train_loader):
            ...
            if (it % train_flags.save_interval == 0 and it != 0) or it == 100:
                ...
                logger.log(f"Saving model in directory... {model_path}")
                ...
                if it in [10**2, 250, 500, 10**3, 10**4, 10**5, 2**5]:
...
if __name__ == "__main__":
    ...
    help="config settings")
    parser.add_argument('--name', type=str, default='vq_vae')
    parser.add_argument('--out_dir', type=str, default='/g/data/po67/anonymous_cat/Trained/VQVAE/')
    ...
    logger.configure(out_dir=meta_args.out_dir, debug=meta_args.debug,time=time)
    ...
    dataset_test_dir = os.path.join(args.data_dir, '')
    ...
    train_dataset = datasets_classes[args.dataset](dataset_train_dir,
    ...
    if train_loader.dataset.eval_flag:
        ...
        eval_dataset = eval_datasets[args.dataset](np.arange(0,len(train_loader.dataset)))

        train(args, train_loader, model, vct_dec, vct_enc, optimizer, eval_dataset, test_loader)
    else:
        train(args, train_loader, model, vct_dec, vct_enc, optimizer, None, None)
