...
The below code is adapted from https://github.com/ThomasMrY/VCT/blob/master/models/visual_concept_tokenizor.py
...
class VCT_Encoder(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.latents = nn.Parameter(torch.randn(num_latents, latent_dim), True) # serve as the concept prototypes
        self.cs_layers = nn.ModuleList([]) # serves as the cross-attention module for the concept prototypes 
        # create norm, attention, norm, feedforward layers
        ...
        # define modules for image attention module
    ...
    def forward(
        ...
        # add positional encoding to image tokens (data)
        ...
        x0 = repeat(self.latents, 'n d -> b n d', b = b) # repeat concept prototypes across batch dim (latents = concept prototypes)
        for i in range(self.depth):
            cross_attn, cross_ff = self.cs_layers[i]

            # cross attention only happens once for Perceiver IO

            x = cross_attn(x0, context = data, mask = mask) + x0 # compute cross-attention (and residual add) between concept prototypes
            x0 = cross_ff(x) + x # feedforward 

            if i != self.depth - 1: # perform update of image tokens 
                self_attn, self_ff = self.layers[i]
                x_d = self_attn(data) + data
                data = self_ff(x_d) + x_d

        return x0 # return processed input (concept tokens)
