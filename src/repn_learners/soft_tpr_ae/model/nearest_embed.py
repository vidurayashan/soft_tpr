...
""" 
The below code is adapted from https://github.com/ThomasMrY/VCT/blob/master/models/nearest_embed.py
...
class NearestEmbed(nn.Module):
    def __init__(self, num_embeddings: int, embedding_dim: int, init_orth: bool=False):
        super().__init__()
        self.weight = nn.Parameter(torch.rand(embedding_dim, num_embeddings))
        init_embeddings(init_orth=init_orth, weights=self.weight.t())

    def forward(self, x, weight_sg=False):
        """Input:
        ---------
        Original method expects: x - (batch_size, emb_size, *)
        New method expects x - (batch_size, n_role_embeddings, emb_dim)
        """
        N, n_embeddings = x.shape[:-1]
        return nearest_embed(x.view(N*n_embeddings, -1).unsqueeze(-1), self.weight.detach() if weight_sg else self.weight)
