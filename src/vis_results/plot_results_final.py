...
def parse_results_from_wandb_logs(mode: str,
    ...
    if 'dis' not in mode:
        ...
        results = dict(filter(lambda x: 'new_ae' not in x[0] or ('new_ae' in x[0] and x[0] in soft_tpr_variants_to_plot), 
                          results.items()))
    #print(f'In parse, results is {results}')
...
def add_vct_results(results_to_plot_standard: Dict, dataset: str, metric: str): 
    """ 
    Running vct training code automatically saves results in json file, parse into required format
    """
    ...
    for iter_key, result_list in d.items():
        for result_dict in result_list: 
            #print(f'Result dict in json is {result_dict}')
...
def box_plot_downstream_model_performance_avr(results_by_dataset, title, models_to_plot, plot_save_path): 
    ...
    fig_e_1, axs_e_1 = plt.subplots(1, 3, figsize=(18, 8)) # one plot per dataset
    plt.subplots_adjust(left=0.06, 
                                right=0.91,
                                bottom=0.1, 
                                top=0.9, 
    ...
    plt.subplots_adjust(left=0.06, 
                                ...
                                wspace=0.38,
                                hspace=0.10)
    fig_s_1, axs_s_1 = plt.subplots(1, 3, figsize=(18, 8)) # one plot per dataset
    plt.subplots_adjust(left=0.06, 
                                right=0.91,
                                bottom=0.1, 
                                top=0.9, 
...
def box_plot_downstream_model_sample_eff(downstream_task, results_by_dataset, title=str, 
    ...
    sns.set_style('darkgrid', {'grid.linestyle': ':'})
    for dataset, results_dict in results_by_dataset.items(): 
        fig_e_1, axs_e_1 = plt.subplots(1, 2, figsize=(15, 10)) # one plot per dataset
        plt.subplots_adjust(left=0.06, 
                                right=0.90,
                                bottom=0.1, 
                                top=0.9, 
                                wspace=0.22,
                                hspace=0.10)
        fig_s_1, axs_s_1 = plt.subplots(1, 2, figsize=(15, 10)) # one plot per dataset
        plt.subplots_adjust(left=0.06, 
                                right=0.90,
                                bottom=0.1, 
                                top=0.9, 
        ...
        fig_e_2, axs_e_2 = plt.subplots(1, 3, figsize=(18, 10)) # one plot per dataset
        plt.subplots_adjust(left=0.05, 
                                right=0.91,
                                bottom=0.1, 
                                top=0.9, 
                                wspace=0.25,
                                hspace=0.10)
        fig_s_2, axs_s_2 = plt.subplots(1, 3, figsize=(18, 10)) # one plot per dataset
        plt.subplots_adjust(left=0.05, 
                                right=0.91,
                                bottom=0.1, 
                                top=0.9, 
...
pretty_models ={ 
        ...
        'new_ae_approx_fillers_concatenated': 'af', 
        'new_ae_z_tpr': 'tpr',
...
SORT_ORDER = {
        ...
        'new_ae_approx_fillers_concatenated': 16, 
        'new_ae_z_tpr': 17,
...
model_to_colour_map = {
    ...
    'new_ae_approx_fillers_concatenated': clrs[0], 
    'new_ae_z_tpr': clrs[8],
